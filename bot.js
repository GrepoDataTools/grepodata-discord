...
client.on('message', (message) => {
    try {
        ...
        switch (main_arg) {
            case 'intel':
            case 'userscript':
                request.get({ url: auth.backend + '/discord/get_index?guild=' + guild_id }, function(
                    error,
                    response,
                    body
                ) {
                    if (response && response.statusCode && response.statusCode === 200) {
                        ...
                        let key = index.key;
                        let userscript =
                            'https://api.grepodata.com/userscript/cityindexer_' + index.userscript + '.user.js';
                        ...
                        if (main_arg === 'userscript') {
                            const embed = new MessageEmbed()
                                ...
                                .setColor(0x18bc9c)
                                .setDescription(
                                    'Help your alliance by contributing to enemy city index [' +
                                        key +
                                        '](https://grepodata.com/indexer/' +
                                        key +
                                        ')!\n' +
                                        '1. Download Tampermonkey for your browser\n' +
                                        '2. [Install this userscript for index ' +
                                        key +
                                        '](' +
                                        userscript +
                                        ')\n' +
                            ...
                            request.get({ url: url }, function(error, response, body) {
                                if (response && response.statusCode && response.statusCode === 200) {
                                    ...
                                    let town_url =
                                        'https://grepodata.com/indexer/town/' +
                                        key +
                                        '/' +
                                        intel.world +
                                        '/' +
                                    ...
                                    const embed = new MessageEmbed()
                                        ...
                                        .setDescription(
                                            'Index: [' +
                                                ...
                                                'Player: [' +
                                                intel.player_name +
                                                '](https://grepodata.com/indexer/player/' +
                                                key +
                                                '/' +
                                                intel.world +
                                                '/' +
                                                intel.player_id +
                                                ') - ' +
                                                'Alliance: [show alliance intel](https://grepodata.com/indexer/alliance/' +
                                                key +
                                                '/' +
                                                intel.world +
                                                '/' +
                                    ...
                                    embed.addField(
                                        'Troop intelligence [town]' + intel.town_id + '[/town]',
                                        troop_intel,
                                        false
                                    );
                                    ...
                                    message.reply(
                                        "Sorry, I found no intelligence for town '" + town_input + "' in index " + key
                                    );
                                }
                            });
                        } else if (isRawTown === false) {
                            const embed = new MessageEmbed()
                                ...
                                .setColor(0x18bc9c)
                                .setDescription(
                                    '[Click here to go to your index](https://grepodata.com/indexer)\n' +
                                ...
                                .addField(
                                    '`!gd intel [town_id]`',
                                    'Shows the intelligence for the given town id',
                                    true
                                );
                            message.channel.send(embed);
                        }
                        ...
                        const embed = new MessageEmbed()
                            ...
                            .setColor(0x18bc9c)
                            .setDescription(
                                'Please set a default index first: `!gd setIndex [index_key]`.\n' +
                ...
                if (typeof parsed.arguments[1] === 'undefined') {
                    ...
                    request.get(opts, function(error, response, body) {
                        if (response && response.statusCode && response.statusCode === 200) {
                            ...
                            message.reply(
                                'Oops, we can not do that right now. Try again later or with different arguments.'
                            );
                        }
                    });
                }
                ...
                request.get(opts, function(error, response, body) {
                    if (response && response.statusCode && response.statusCode === 200) {
                        ...
                        const embed = new MessageEmbed()
                            ...
                            .setColor(0x18bc9c)
                            .setDescription(
                                'Showing player points gained on ' +
                                    scoreboard.date +
                                    (main_arg === 'today' ? ' before ' + scoreboard.time : ' (**yesterday**)')
                            );
                        if (main_arg === 'today') {
                        ...
                        scoreboard.att.slice(0, 10).forEach(function(player, i) {
                            att_txt +=
                                ...
                                ' - ' +
                                '[' +
                                escapeMarkdown(player.n) +
                                ']' +
                                '(https://grepodata.com/player?world=' +
                                scoreboard.world +
                                '&id=' +
                                player.i +
                                ')' +
            ...
            default:
                const embed = new MessageEmbed()
                    ...
                    .addField(
                        '`!gd today [server]`',
                        'Shows the scoreboard for the given server. E.g. `!gd today nl74` will show the scoreboard for nl74',
                        false
                    )
                    .addField(
                        '`!gd setServer [server]`',
                        'Set the default server to [server]. E.g. `!gd setServer nl74` will change the default server to nl74',
                        false
                    )
                ...
                embed.addField(
                    '\u200B',
                    '*If you have set a default index key, GrepoData Bot will respond to town BB code.*',
                    false
                );
                embed.addField(
                    '\u200B',
                    '***Disclaimer: GrepoData is an unofficial 3rd party site. Grepolis is a browser game by InnoGames GmbH.***',
                    false
                );

...
exports.run = async (client, message) => {
    ...
    if (!isNaN(message.content)) {
        ...
        const heatmapSearchUri = `${process.env.BACKEND_URL}/player/search?query=${encodeURI(
            message.content
        )}&from=0&size=1&sql=true&server=${serverName}`;
            ...
            .then(async (response) => {
                ...
                if (!player.heatmap || player.heatmap.length <= 0) {
                    embed.setDescription(
                        `Sorry, it appears [${player.name}](${process.env.FRONTEND_URL}/player?world=${player.world}&id=${player.id}) is not active enough to create a heatmap for.`
                    );
                } else {
                    const heatmap = await createHeatmapChartForPlayer(player);
                    embed.attachFiles([heatmap.image]).setImage('attachment://heatmap.jpg');
                }
                ...
                message.channel.send({ embed }).catch((error) => {
                    ...
                    if (error.message === 'Missing Permissions') {
                        message.channel.send(
                            `Sorry, I can not do that. This Discord channel does not allow me to upload attachments :(`
                        );
                    } else {
                        message.channel.send(`Sorry, I can not do that right now`);
                    }

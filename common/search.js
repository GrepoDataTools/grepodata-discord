    ...
    try {
        ...
        if (!playername) {
            await sendInteractionResponse(interaction, {
                content: 'Please enter a player name to search for. For example: `/search John Doe`.'
            });
            return;
        } else {
            ...
            try {
                ...
                await sendInteractionResponse(interaction, {
                    content: `Could not find any players with a name matching **${playername}**. Use command \`/world [WORLD]\` to change the preferred server for this guild.`
                });
            }
        }
